...
//m_emitter.reset(new Emitter(ngl::Vec3::zero(),15, &m_cam));
//glPointSize(5);
 startTimer(200);
...
//if you remove this your boid is gone! why ? no idea
...
//for loop store all the boids in the std::vector<Boid> m_boidArray;
 ...
 // m_emitter->draw();
  ...
  ngl::Transformation t;
   ...
   ngl::Vec3 m_dir=rng->getRandomVec3();

/*
  Particle b1(m_pos,m_dir,m_emitter);
  b1.draw();
  t.setPosition(-10.0f,0.0f,0.0f);
  ngl::Mat4 MVP= m_mouseGlobalTX*t.getMatrix()*m_cam.getVPMatrix();
  shader->setUniform("MVP",MVP);

  Boid b2(m_pos,m_dir,m_emitter);
  b2.draw();
